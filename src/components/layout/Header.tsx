  ...
  return (
    <header className="fixed inset-x-0 top-0 z-50 border-b border-brown/10 bg-cream/90 backdrop-blur-xl">
      <div className="container-shell flex h-20 items-center justify-between">
        <Link href="/#inicio" className="font-display text-xl text-ink" aria-label={`${settings.name}, início`}>
          {settings.name}
        </Link>
        <nav className="hidden items-center gap-5 xl:flex" aria-label="Navegação principal">
          ...
          <a href={settings.instagram} target="_blank" rel="noopener noreferrer" className="nav-link inline-flex items-center gap-1" aria-label="Instagram — abre em nova aba"><Instagram size={14} /></a>
          {settings.tiktok && <a href={settings.tiktok} target="_blank" rel="noopener noreferrer" className="nav-link inline-flex items-center gap-1" aria-label="TikTok — abre em nova aba"><Music2 size={14} /></a>}
        </nav>
        ...
        <div className="flex items-center gap-2 xl:hidden">
          <a href={whatsappUrlFor(settings.whatsapp, settings.whatsappMessage)} target="_blank" rel="noopener noreferrer" className="grid size-10 place-items-center rounded-full bg-[#25D366] text-white shadow sm:hidden" aria-label="Solicitar orçamento pelo WhatsApp — abre em nova aba">
