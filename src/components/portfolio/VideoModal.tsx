    ...
    <div className="fixed inset-0 z-[70] grid place-items-center bg-ink/85 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-labelledby="video-title" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
      <div className="relative grid max-h-[92vh] w-full max-w-4xl overflow-auto rounded-3xl bg-cream shadow-2xl md:grid-cols-[minmax(280px,.75fr)_1fr]">
        ...
        <div className="relative aspect-[9/16] min-h-[420px] overflow-hidden bg-brown">
          {!item.videoUrl ? <><Image src={resolveMediaPath(item.thumbnail)} alt="" fill sizes="420px" className="object-cover opacity-50" /><div className="absolute inset-0 grid place-items-center p-8 text-center"><div><p className="eyebrow text-white">Conteúdo demonstrativo</p><p className="mt-4 font-display text-3xl text-white">Vídeo oficial será inserido aqui</p></div></div></> : item.source === "local" ? <video src={resolveMediaPath(item.videoUrl)} poster={resolveMediaPath(item.thumbnail)} controls playsInline preload="metadata" className="size-full object-cover" /> : item.source === "youtube" || item.source === "vimeo" ? <iframe src={embedUrl(item)} title={item.title} className="size-full" allow="accelerometer; encrypted-media; picture-in-picture" allowFullScreen /> : item.source === "instagram" ? <><Image src={resolveMediaPath(item.thumbnail)} alt={item.title} fill sizes="420px" className="object-cover" /><div className="absolute inset-0 grid place-items-center bg-ink/40 p-8 text-center"><div><Play size={48} className="mx-auto text-white" /><a href={item.externalUrl ?? item.videoUrl} target="_blank" rel="noopener noreferrer" className="button-light mt-6 inline-flex items-center gap-2">Ver no Instagram <ExternalLink size={16} /></a></div></div></> : <div className="grid size-full place-items-center p-8 text-center text-white"><a href={item.videoUrl} target="_blank" rel="noopener noreferrer" className="button-light">Assistir na plataforma <ExternalLink size={16} /></a></div>}
        </div>
